...
async function enviarMensagem(telefone, nomeEmpresa, etapaAtual, linha) {
  try {
    // Resolve número: tenta getNumberId para garantir @c.us correto
    let telefoneResolvido = telefone;
    try {
      const numeroLimpo = telefone.replace(/@.+/, "");
      const wid = await client.getNumberId(numeroLimpo);
      if (wid) {
        telefoneResolvido = wid._serialized;
        console.log(`✅ Número resolvido: ${telefoneResolvido}`);
      } else {
        console.warn(`⚠️ Número não encontrado no WhatsApp: ${numeroLimpo}`);
        await atualizarStatus(linha, "ERRO", etapaAtual, "Número não encontrado no WhatsApp");
        return false;
      }
    } catch(e) {
      console.warn(`⚠️ Não foi possível validar número, tentando direto: ${e.message}`);
    }
    const chat = await client.getChatById(telefoneResolvido);
    await chat.sendStateTyping();
    await delay(3000);

    if (etapaAtual === 0) {
      await client.sendMessage(telefoneResolvido, `Olá, tudo bem? 👋`);
      await atualizarStatus(linha, "AGUARDANDO_RESPOSTA", 1, "MSG 1 enviada");
      console.log(`📨 MSG 1 → ${nomeEmpresa}`);
    }
    else if (etapaAtual === 1) {
      await client.sendMessage(telefoneResolvido, `Queria falar com a responsável pela agenda ou atendimento, por favor.`);
      await atualizarStatus(linha, "AGUARDANDO_RESPOSTA", 2, "MSG 2 enviada");
      console.log(`📨 MSG 2 → ${nomeEmpresa}`);
    }
    else if (etapaAtual === 2) {
      await client.sendMessage(telefoneResolvido, `Boa \n\nPesquisei por cílios na sua região e vi que tem bastante gente procurando, mas muitos perfis não estão aproveitando esse movimento.\n\nQueria te mostrar algo rápido sobre isso, pode ser?`);
      await atualizarStatus(linha, "AGUARDANDO_RESPOSTA", 3, "MSG 3 enviada");
      console.log(`📨 MSG 3 → ${nomeEmpresa}`);
    }
    else if (etapaAtual === 3) {
      await client.sendMessage(telefoneResolvido, `Vou te mostrar uma ferramenta. Você consegue ver em tempo real:\n\nhttps://analise-perfil-google.lovable.app/\n\nDepois me fala o que apareceu aí pra você que te explico melhor!`);
      console.log(`📨 MSG 4 → ${nomeEmpresa}`);
      await delay(30000);
      try {
        const audio = MessageMedia.fromFilePath("/app/audio4.ogg");
        await client.sendMessage(telefoneResolvido, audio);
    ...
    if (error.message && error.message.includes("invalid wid")) {
      console.log(`🔄 Tentando resolver número via getNumberId...`);
      try {
        const numeroLimpo = telefone.replace("@c.us", "").replace("@lid", "");
        const wid = await client.getNumberId(numeroLimpo);
        if (wid) {
          console.log(`✅ Número resolvido: ${wid._serialized}`);
          await atualizarStatus(linha, "NAO_INICIADO", etapaAtual, "Número corrigido, tentar novamente");
          return false;
        }
      } catch(e2) {
        console.error("❌ Não foi possível resolver número:", e2.message);
      }
    }
...
// PROCESSAR RESPOSTA
// =====================================
async function processarResposta(numeroReal, texto, nomeContato) {
  try {
    ...
    // Extrai apenas os dígitos do número recebido para comparação
    const digitosRecebido = numeroReal.replace(/\D/g, '');

    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      if (!row[1]) continue;

      // Extrai apenas os dígitos do número da planilha
      const digitosPlanilha = row[1].toString().replace(/\D/g, '');

      // Compara pelo sufixo: os últimos 9 dígitos devem bater
      // Isso resolve diferenças de DDI (55) e formato
      const sufixoRecebido = digitosRecebido.slice(-9);
      const sufixoPlanilha = digitosPlanilha.slice(-9);

      console.log(`🔎 Comparando: recebido=${sufixoRecebido} | planilha=${sufixoPlanilha}`);

      if (sufixoRecebido === sufixoPlanilha) {
        const status = row[2] || "";
        const etapa = parseInt(row[3]) || 0;
        ...
        if (status === "AGUARDANDO_RESPOSTA" && etapa < 4) {
          ...
          await delay(15000);
          await enviarMensagem(telefonePlanilha, nomeEmpresa, etapa, i + 1);
        } else {
    ...
    console.log(`⚠️ Nenhum lead encontrado para o número: ${numeroReal} (dígitos: ${digitosRecebido})`);
  } catch (error) {
...
async function iniciarBot() {
  ...
  client.on("message", async (msg) => {
    ...
    let nomeContato = msg.from;
    let numeroReal = null;

    try {
      const contact = await msg.getContact();
      nomeContato = contact.pushname || contact.name || msg.from;
      // contact.number sempre retorna o número real no formato DDI+DDD+número
      numeroReal = contact.number || null;
    } catch (_) {}

    // Se não conseguiu pegar o número real pelo contact, tenta pelo msg.from
    if (!numeroReal) {
      numeroReal = msg.from.replace("@c.us", "").replace("@lid", "");
    }

    console.log(`📥 Msg de ${nomeContato} | Número real: ${numeroReal} | Texto: "${texto.substring(0, 40)}"`);
    await processarResposta(numeroReal, texto, nomeContato);
  });
